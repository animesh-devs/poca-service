...
async def create_message(
    ...
    if current_user.role == UserRole.ADMIN:
        ...
        # Verify sender ID matches the doctor's ID
        if message_data.sender_id != doctor.id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=create_error_response(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        # Verify sender ID matches the patient's ID
        if message_data.sender_id != patient.id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=create_error_response(
                    status_code=status.HTTP_400_BAD_REQUEST,
